...
import re
import numpy
def reverse_complement(seq, rev_cmplmnt_bases, rev_cmplmnt_map):
...
class SAMParserPlugin:
 ...
 def output(self, outputfile):
  ...
  SR_filename = PyPluMA.prefix()+"/"+self.parameters["SR_filename"] 
  sam_filename = PyPluMA.prefix()+"/"+self.parameters["sam_filename"] 
  nav_filename = PyPluMA.prefix()+"/"+self.parameters["nav_filename"] 
  one_line_per_alignment = self.parameters["one_line_per_alignment"]
  ################################################################################
  ...
  for line in sam_file:
    ...
    if (SR_name != SR_seq_readname):
        ...
        SR_seq_rvs_cmplmnt = reverse_complement(SR_seq, rev_cmplmnt_bases, rev_cmplmnt_map)
